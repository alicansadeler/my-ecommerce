...
const Blog = () => {
  ...
  const blogCard = (img) => (
    <div className="bg-white rounded-lg overflow-hidden shadow-lg">
      <img src={img} alt="blog1" className="w-full h-[300px] object-cover" />
      ...
      <div className="text-left px-4 flex flex-col gap-4 font-semibold text-bgGray text-sm">
  ...
  return (
    <div className="flex flex-col gap-10 p-4 overflow-x-hidden">
      <div className="w-[338px]">{blogCard(img1)}</div>
      <div className="w-[338px]">{blogCard(img2)}</div>
    </div>
  );
};
